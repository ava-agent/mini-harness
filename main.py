...
Usage:
    ...
    python main.py --project /path/to/repo  # Set project context
    python main.py --session 2026-03-30-143022  # Resume session
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Mini Harness — Landing Knowledge Assistant")
    parser.add_argument("--project", type=str, default="", help="Path to project to analyze")
    parser.add_argument("--session", type=str, default="", help="Resume a saved session")
    args = parser.parse_args()

    # --- Init components ---
    print(f"{_C.BOLD}Mini Harness v0.1 — Landing Knowledge Assistant{_C.RESET}")
    ...
    try:
        ...
        print(f"{_C.GRAY}Set GLM_API_KEY environment variable or copy .env.example to .env{_C.RESET}")
        sys.exit(1)

    memory = MemoryStore(session_id=args.session)
    safety = SafetyGuard()

    project_path = os.path.abspath(args.project) if args.project else ""
    if project_path:
